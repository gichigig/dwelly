...
import 'package:dwelly/core/models/helper_job.dart';
import 'package:dwelly/core/services/helper_job_service.dart';
  ...
  Future<void> _updateJobStatus(int jobId, String action) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Job $action successful'), backgroundColor: Colors.green),
      );
      _loadJobs();
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    Text('Helper: ${job.helper.fullName}', style: const TextStyle(fontWeight: FontWeight.bold)),
                                    Chip(
                                      label: Text(job.status.value, style: const TextStyle(fontSize: 12)),
                                      backgroundColor: job.status.value == 'ACTIVE' ? Colors.green.shade100 : Colors.grey.shade200,
                                    ),
                                  ],
                                ),
