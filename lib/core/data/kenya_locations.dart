  ...
  static String? _getCountyForLocation(String location) {
    ...
    for (final entry in wardsByConstituency.entries) {
      if (entry.value.contains(location))
        return _getCountyForConstituency(entry.key);
    }
    return null;
  }

  static List<String> getConstituencies(String county) =>
      constituenciesByCounty[county] ?? [];
  static List<String> getWards(String constituency) =>
      wardsByConstituency[constituency] ?? [];
}
  ...
  String get subtitle {
    switch (type) {
      case LocationType.county:
        return 'County';
      case LocationType.constituency:
        return 'Constituency in $county County';
      case LocationType.ward:
        return 'Ward in $constituency, $county';
      case LocationType.area:
